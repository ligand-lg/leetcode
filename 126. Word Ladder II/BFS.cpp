...
class Solution {
  bool one_diff(const string &s1, const string &s2) {
 ...
 public:
  vector<vector<string>> findLadders(string beginWord, string endWord,
    ...
    paths.emplace(1, beginWord);
    int level = 1;
    int min_ladder_length = INT_MAX;
    ...
    while (!paths.empty()) {
      ...
      paths.pop();
      if (path.size() > level) {
        if (path.size() > min_ladder_length)
          break;
        level = path.size();
